...
fn render_process_service_state_card(f: &mut Frame, app: &App, area: Rect) {
    ...
    let text = vec![
        ...
        Line::from(vec![
            Span::styled("• Network Telemetry: ", Style::default().fg(theme.warning).add_modifier(Modifier::BOLD)),
            Span::styled("Explore Ingress/Egress meters, open ports & socket maps in Tab [2]", theme.dim_style()),
        ]),
    ];
...
fn render_health_diagnostics_card(f: &mut Frame, app: &App, area: Rect) {
    ...
    lines.push(Line::from(vec![
        Span::styled("󰃢 System Cache: ", Style::default().fg(theme.warning).add_modifier(Modifier::BOLD)),
    ...
    let advisory_text = if zombie_procs > 0 {
        Line::from(vec![
            ...
            Span::styled(format!("Found {} zombie processes. Inspect parent processes in Tab [3].", zombie_procs), Style::default().fg(theme.danger)),
        ])
    } else if failed_services > 0 {
        Line::from(vec![
            Span::styled("💡 Health Advisory: ", Style::default().fg(theme.danger).add_modifier(Modifier::BOLD)),
            Span::styled(format!("{} systemd service(s) in failed state. Review logs in Tab [5].", failed_services), Style::default().fg(theme.danger)),
        ])
        ...
        Line::from(vec![
            ...
            Span::styled(format!("Reclaimable disk cache available ({}). Press '4' to run cleaner.", format_bytes(cleanable_bytes)), Style::default().fg(theme.warning)),
        ])
    } else {
        Line::from(vec![
