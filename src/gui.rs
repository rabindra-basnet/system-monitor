...
type GtkWindowSetDefaultSizeFn =
    unsafe extern "C" fn(window: GtkWidget, width: c_int, height: c_int);
type GtkWindowSetPositionFn = unsafe extern "C" fn(window: GtkWidget, position: c_int);
type GtkWindowSetIconFromFileFn = unsafe extern "C" fn(
    window: GtkWidget,
...
type GtkCssProviderLoadFromDataFn = unsafe extern "C" fn(
    provider: *mut c_void,
    data: *const c_char,
    length: c_long,
...
type GtkStyleContextAddProviderForScreenFn =
    unsafe extern "C" fn(screen: GdkScreen, provider: *mut c_void, priority: u32);
...
unsafe extern "C" fn on_window_destroy_or_child_exit(
    _widget: *mut c_void,
    _user_data: *mut c_void,
) {
    let gtk_lib = libc::dlopen(c"libgtk-3.so.0".as_ptr() as *const c_char, libc::RTLD_LAZY);
    if !gtk_lib.is_null() {
        let sym = libc::dlsym(gtk_lib, c"gtk_main_quit".as_ptr() as *const c_char);
        if !sym.is_null() {
...
pub fn try_launch_native_gui(forward_args: &[String]) -> Result<(), String> {
    unsafe {
        ...
        let vte_lib = libc::dlopen(
            c"libvte-2.91.so.0".as_ptr() as *const c_char,
            libc::RTLD_LAZY,
        );
        if vte_lib.is_null() {
        ...
        let glib_lib = libc::dlopen(
            c"libglib-2.0.so.0".as_ptr() as *const c_char,
            libc::RTLD_LAZY,
        );
        let gobject_lib = libc::dlopen(
            c"libgobject-2.0.so.0".as_ptr() as *const c_char,
            libc::RTLD_LAZY,
        );

        // Resolve symbols
        let gtk_init: GtkInitFn =
            std::mem::transmute(libc::dlsym(gtk_lib, c"gtk_init".as_ptr() as *const c_char));
        let gtk_window_new: GtkWindowNewFn = std::mem::transmute(libc::dlsym(
            gtk_lib,
            c"gtk_window_new".as_ptr() as *const c_char,
        ));
        let gtk_window_set_title: GtkWindowSetTitleFn = std::mem::transmute(libc::dlsym(
            gtk_lib,
            c"gtk_window_set_title".as_ptr() as *const c_char,
        ));
        let gtk_window_set_default_size: GtkWindowSetDefaultSizeFn =
            std::mem::transmute(libc::dlsym(
                gtk_lib,
                c"gtk_window_set_default_size".as_ptr() as *const c_char,
            ));
        let gtk_window_set_position: GtkWindowSetPositionFn = std::mem::transmute(libc::dlsym(
            gtk_lib,
            c"gtk_window_set_position".as_ptr() as *const c_char,
        ));
        let gtk_window_set_icon_from_file: GtkWindowSetIconFromFileFn =
            std::mem::transmute(libc::dlsym(
                gtk_lib,
                c"gtk_window_set_icon_from_file".as_ptr() as *const c_char,
            ));
        let gtk_container_add: GtkContainerAddFn = std::mem::transmute(libc::dlsym(
            gtk_lib,
            c"gtk_container_add".as_ptr() as *const c_char,
        ));
        let gtk_widget_show_all: GtkWidgetShowAllFn = std::mem::transmute(libc::dlsym(
            gtk_lib,
            c"gtk_widget_show_all".as_ptr() as *const c_char,
        ));
        let gtk_main: GtkMainFn =
            std::mem::transmute(libc::dlsym(gtk_lib, c"gtk_main".as_ptr() as *const c_char));

        let vte_terminal_new: VteTerminalNewFn = std::mem::transmute(libc::dlsym(
            vte_lib,
            c"vte_terminal_new".as_ptr() as *const c_char,
        ));
        let vte_terminal_set_cursor_blink_mode: VteTerminalSetCursorBlinkModeFn =
            std::mem::transmute(libc::dlsym(
                vte_lib,
                c"vte_terminal_set_cursor_blink_mode".as_ptr() as *const c_char,
            ));
        let vte_terminal_set_scrollback_lines: VteTerminalSetScrollbackLinesFn =
            std::mem::transmute(libc::dlsym(
                vte_lib,
                c"vte_terminal_set_scrollback_lines".as_ptr() as *const c_char,
            ));
        let vte_terminal_set_mouse_autohide: VteTerminalSetMouseAutohideFn =
            std::mem::transmute(libc::dlsym(
                vte_lib,
                c"vte_terminal_set_mouse_autohide".as_ptr() as *const c_char,
            ));
        let vte_terminal_spawn_async: VteTerminalSpawnAsyncFn = std::mem::transmute(libc::dlsym(
            vte_lib,
            c"vte_terminal_spawn_async".as_ptr() as *const c_char,
        ));

        let g_signal_connect_data: GSignalConnectDataFn = std::mem::transmute(libc::dlsym(
            gobject_lib,
            c"g_signal_connect_data".as_ptr() as *const c_char,
        ));

        // Set application names
        if !glib_lib.is_null() {
            let prg_sym = libc::dlsym(glib_lib, c"g_set_prgname".as_ptr() as *const c_char);
            if !prg_sym.is_null() {
                let g_set_prgname: GSetPrgnameFn = std::mem::transmute(prg_sym);
                g_set_prgname(c"stasis".as_ptr() as *const c_char);
            }
            let app_sym = libc::dlsym(
                glib_lib,
                c"g_set_application_name".as_ptr() as *const c_char,
            );
            if !app_sym.is_null() {
                let g_set_application_name: GSetApplicationNameFn = std::mem::transmute(app_sym);
                g_set_application_name(c"Stasis".as_ptr() as *const c_char);
            }
        }
        ...
        // Set Dark Theme CSS
        let css_new_sym = libc::dlsym(gtk_lib, c"gtk_css_provider_new".as_ptr() as *const c_char);
        let screen_get_sym =
            libc::dlsym(gtk_lib, c"gdk_screen_get_default".as_ptr() as *const c_char);
        let add_provider_sym = libc::dlsym(
            gtk_lib,
            c"gtk_style_context_add_provider_for_screen".as_ptr() as *const c_char,
        );
        let load_data_sym = libc::dlsym(
            gtk_lib,
            c"gtk_css_provider_load_from_data".as_ptr() as *const c_char,
        );

        if !css_new_sym.is_null()
            && !screen_get_sym.is_null()
            && !add_provider_sym.is_null()
            && !load_data_sym.is_null()
        {
            ...
            let gtk_style_context_add_provider_for_screen: GtkStyleContextAddProviderForScreenFn =
                std::mem::transmute(add_provider_sym);
            let gtk_css_provider_load_from_data: GtkCssProviderLoadFromDataFn =
                std::mem::transmute(load_data_sym);
        ...
        g_signal_connect_data(
            window,
            destroy_sig.as_ptr(),
            on_window_destroy_or_child_exit,
            ptr::null_mut(),
            ptr::null(),
            0,
        );
        ...
        let mut child_argv_ptrs: Vec<*const c_char> =
            child_argv_cstrings.iter().map(|s| s.as_ptr()).collect();
